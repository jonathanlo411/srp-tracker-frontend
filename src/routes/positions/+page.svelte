<script lang='ts'>
  ...
  let positionChanges: Record<string, PositionChange[]> = {};
  ...
  onMount(async () => {
    for (let leaderboard of leaderboards) {
      ...
      positionChanges[leaderboard] = calculatePositionChanges(leaderboardData);
    }
...
<svelte:head>
  <title>SRP Tracker | Position Changes</title>
</svelte:head>
  ...
  <div id='base' class={isLoaded ? 'fade-in' : ''}>
    <section id='main'>
      <div id='heading'>
        ...
        <p>This page contains all information about postion increases on all of the leaderboards (with "Combined" being all of the individual ones added up). Do note that the position change data is only for the last 24 hours.</p>
      </div>
      ...
      <div id='controls'>
        ...
        <div id='items-control'>
          <label for="itemsToShow">Number of items to show:</label>
      ...
      {#each visibleLeaderboards as leaderboard}
        <div class='leaderboard'>
          ...
          <div class='leaderboard-list'>
            {#each positionChanges[leaderboard]?.slice(0, itemsToShow) as change}
